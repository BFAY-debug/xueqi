...
- **安全加固**：CORS 白名单、安全响应头（CSP + HSTS + X-Frame-Options 等）、密码环境变量必填、Service Token 角色降级、Redis fail-closed、httpOnly Cookie、验证码防暴力破解
- **内容安全**：DFA 敏感词过滤（全服务覆盖）、输入预处理（大小写/全角/繁简/特殊字符对抗）、126 条预设敏感词、DOMPurify 防XSS、sanitize-html 白名单过滤
...
| 缓存      | Redis 7（ioredis，AOF 持久化，密码认证）                                              | 7.x  |
| 认证      | JWT (jsonwebtoken) + bcryptjs（accessToken 2h + refreshToken 7d，httpOnly Cookie）   | —    |
...
├── docker-compose.prod.yml     # 生产环境覆盖（端口隔离、Redis 7、NODE_ENV）
├── .env                        # 生产环境变量
├── .env.example                # 环境变量模板
├── .dockerignore               # Docker 构建排除
├── 技术文档.md                  # 本文档
├── 部署指南.md                  # Cloudflare + Let's Encrypt 部署步骤
├── 开发计划.md                  # 开发路线图
│
├── database/
│   └── init/
...
登录 → 返回 accessToken(2h) + refreshToken(7d) + 用户信息(含 avatar_url)
     → 开发环境: localStorage 存储 token + refreshToken
     → 生产环境: accessToken 存内存, refreshToken 存 httpOnly Cookie (JS不可读)
     → Axios 拦截器自动附加 Authorization: Bearer <accessToken> (withCredentials: true)
...
**httpOnly Cookie（生产环境）**: Refresh Token 通过服务端 `Set-Cookie` 设置为 `httpOnly; Secure; SameSite=Strict`，JavaScript 无法读取，防止 XSS 窃取。开发环境仍使用 localStorage，通过 `import.meta.env.PROD` 自动切换。

**Redis Fail-Closed**: 所有依赖 Redis 的安全机制（登录锁定、Refresh Token 白名单、速率限制）在 Redis 不可用时拒绝操作返回 503，而非跳过检查。

**账户锁定**: 连续 5 次登录失败后锁定 15 分钟，计数器存储在 Redis 中。

**验证码**: 注册和登录均需 SVG 图形验证码，一次性使用，5 分钟过期。

**Token 轮换**: Refresh Token 每次使用后自动轮换（旧 Token 删除，新 Token 颁发），Redis 白名单机制确保已注销的 Token 不可重用。
...
**生产环境 Nginx** (`frontend/nginx.prod.conf`): 在 HTTPS 配置中额外添加 CSP + HSTS。
...
| 应用层限流     | 发帖 2/min 20/day, 评论 5/min 50/day | `services/shared/src/rateLimiter.js` |
...
- **评论防XSS**: `escapeHtml()` 转义 `<>&"` 字符，@提及链接安全渲染
- **SQL 注入防护**: WHERE 条件使用 mysql2 参数化查询（`?` 占位符），LIMIT/OFFSET 使用 `parseInt()` 强制整数转换
- **服务端内容过滤**: `sanitize-html` 白名单过滤帖子/评论内容（allowedTags + allowedAttributes + allowedSchemes）
- **生产端口隔离**: `docker-compose.prod.yml` 关闭内部服务端口映射，仅暴露 80/443
...
- **请求拦截器**: 自动附加 `Authorization: Bearer <token>`，全局 `withCredentials: true` 支持 Cookie
- **响应拦截器**: `success === false` 时 reject 并携带中文错误消息
- **401 处理**: 自动用 refreshToken 刷新（生产环境通过 httpOnly Cookie 自动传输，`window.__refreshPromise` 并发去重），失败则 logout
...
所有 7 个容器运行，安全头仅设置基础 4 项，Token 使用 localStorage，内部服务端口可从宿主机访问。

### 生产部署（Cloudflare + Docker）

详见 [`部署指南.md`](部署指南.md)，核心差异：

```bash
# 生产模式启动（关闭端口暴露，启用 CSP/HSTS，Redis 7.x，httpOnly Cookie）
...
| Redis 版本 | 7-alpine | 7-alpine |
| 内部服务端口 | 暴露到宿主机 | 关闭，仅 Docker 内部网络 |
| HTTPS | 无 | Let's Encrypt + Cloudflare |
| 安全头 | 4 项基础头 | + CSP + HSTS |
| Token 存储 | localStorage | httpOnly Cookie（Refresh Token） |
| CORS_ORIGIN | localhost | 生产域名 |
| 密钥 | 开发用弱密钥 | 64 字节随机密钥 |
...
MYSQL_PORT=3306
MYSQL_ROOT_PASSWORD=root123           # ⚠️ 生产环境必须更换为强随机密码
MYSQL_DATABASE=xueqi_db

# Redis
REDIS_HOST=xueqi-redis
REDIS_PORT=6379
REDIS_PASSWORD=redis123               # ⚠️ 生产环境必须更换为强随机密码

# CORS 白名单
CORS_ORIGIN=http://localhost,http://localhost:80

# JWT
JWT_SECRET=your-secret-key-change-in-production   # ⚠️ 生产环境必须更换为 64 字节随机密钥
JWT_EXPIRES_IN=2h
...
| 21  | httpOnly Cookie（生产）/ localStorage（开发）        | 生产防 XSS 窃取 Token，开发保持便利               |
| 22  | Redis fail-closed 而非 fail-open                    | 安全优先：Redis 故障时拒绝操作而非跳过安全检查     |
| 23  | LIMIT/OFFSET 用 parseInt 而非参数化                  | mysql2 execute() 不支持 LIMIT ? 占位符，parseInt 安全转换 |
| 24  | 生产/开发环境自动切换（NODE_ENV + import.meta.env） | CSP/HSTS/Cookie/端口 仅生产启用，本地开发不受影响  |
