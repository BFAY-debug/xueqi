...
| **管理后台** | 文章管理（分类/作者/状态/关键词筛选）、评论管理、用户管理、座位管理、反馈管理、志愿者审核、审核日志、系统统计（ECharts 图表） |
...
| 共享模块  | `xueqi-shared`（DB、Redis、JWT、日志、错误处理、响应格式）        | —    |
...
│   │   ├── package.json
│   │   └── src/
│   │       ├── app.js          # Express 服务入口 (路由挂载、CORS 白名单)
...
│   │   ├── package.json
│   │   └── src/
│   │       ├── app.js          # Express + Socket.IO 服务端
│   │       ├── socket.js       # Socket.IO 事件处理器 (541 行, 33 个事件)
...
│           ├── controllers/
│           └── services/       # postService, commentService, bookmarkService, proposalService, mentionService
│
└── frontend/
